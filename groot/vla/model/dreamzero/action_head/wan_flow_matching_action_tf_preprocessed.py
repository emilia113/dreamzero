...
class WANPolicyHead(ActionHead):
    ...
    def set_trainable_parameters(self, tune_projector: bool, tune_diffusion_model: bool):
        ...
        # [wjy] 没有text_encoder，所以不需要设置为False
    ...
    def set_frozen_modules_to_eval_mode(self):
        ...
        if self.training:
            if not self.tune_diffusion_model:
                self.model.eval()
            # [wjy] 没有text_encoder，所以不需要设置为eval
            # self.text_encoder.eval()
            self.image_encoder.eval()
            self.vae.eval()
    
    # [wjy] 推理时使用，此python文件只用于训练，可以注释掉。推理时使用的是原代码groot.vla.model.dreamzero.action_head.wan_flow_matching_action_tf.WANPolicyHead，保留了该函数。因此注释掉该代码不影响推理
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        # 直接使用预计算的 prompt_emb，不再调用 text encoder
        prompt_embs = data["prompt_emb"].to(dtype=torch.bfloat16)
    ...
    def lazy_joint_video_action(self, backbone_output: BatchFeature, action_input: BatchFeature, latent_video: torch.Tensor | None = None) -> BatchFeature:
        ...
        # 使用预计算的 prompt_emb 检测语言是否变化
        prompt_emb_input = data["prompt_emb"].to(dtype=torch.bfloat16)
        if self.language is None:
            print("language is None, reset current_start_frame to 0")
            self.language = prompt_emb_input
            self.current_start_frame = 0
        elif not torch.equal(self.language, prompt_emb_input):
        ...
        # 直接使用预计算的 prompt_emb，不再调用 text encoder
        prompt_emb = prompt_emb_input.to(self._device)
        prompt_embs = [prompt_emb]
        if self.cfg_scale != 1.0:
            # negative prompt: 用全零 embedding（与 encode_prompt 对空文本的效果一致）
            prompt_embs.append(torch.zeros_like(prompt_emb))
