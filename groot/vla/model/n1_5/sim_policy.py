...
class GrootSimPolicy(BaseGrootSimPolicy):
    def __init__(
        ...
        # GrootSimPolicy 只在推理时使用，若 config.json 中 action head 是 preprocessed 版本则自动替换为原版
        ...
        old_cfg_tgt = ah_cfg.get("config", {}).get("_target_", "") if isinstance(ah_cfg.get("config"), dict) else ""
        if "preprocessed" in old_tgt or "preprocessed" in old_cfg_tgt:
            import shutil
            shutil.copy2(config_json_path, model_dir / "config.json.bak")
            print(f"[sim_policy] Detected preprocessed action head, replacing with original")
            ah_cfg["_target_"] = old_tgt.replace("_preprocessed", "")
            if isinstance(ah_cfg.get("config"), dict):
                ah_cfg["config"]["_target_"] = old_cfg_tgt.replace("_preprocessed", "")
